...
*Continuous record of what's built, what's next, and what's parked. Updated after the **edit slice, Step E/F, and Capture v2 all shipped** — the writable-app arc is complete. Pairs with `RenitaOS-Conventions.md` (rules), `RenitaOS-Backend-Notes-Template.md` (schema), and `RenitaOS-Edit-Slice-Build-Brief.md` (the edit-slice implementation contract + Appendix A/B fix log).*
...
*The writable-app arc (edit slice · Step E · Step F · Capture v2) is **done**. Focus shifts from making the app writable to building out the remaining screens.*

1. **Today / This Week / This Month dashboard** — the next build, and the heaviest screen (≈3 queries + 2 derivations + Time Slot writes). It composes cards and drill-downs that now exist and are editable. **Prioritized ahead of Weekly Review** — Today has no interim fallback, whereas the old standalone app still runs weekly reviews until they're migrated. *This is a design conversation, not a fix loop — start it fresh.* Open: comparison baseline + metric definitions; **Week-start = Sunday** as a shared constant so week math can't drift per screen.
2. **Weekly Review** — port the existing standalone dashboard into the framed shell + reskin to the palette. Banked: the WREF reflection note, the 1% goal. Open: comparative-by-domain baseline, metric definitions, mid-week/pre-freeze behavior, past-week navigation.
