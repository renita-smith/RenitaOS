...
- **Today / Tomorrow / This Week / This Month dashboard** (`#/today`, per the standalone Today Dashboard Build Brief) — the 4-query (`Promise.all`) + 2-derivation composition screen: the shared local Sunday-anchored date module; the overdue strip; the Today/Tomorrow time-slot board (Bank + 3 columns) with desktop drag and a tap-to-pick alternative, writing `Time Slot` (whole-array replace via `resolveOptionValue`) and rescheduling via `Do Date`; the completion invariant (removes the card from its bucket + live-updates the header/week/month counts); the This Week list (Sun→Sat, day-tagged, no day-header grouping) and This Month section (project deadlines + a domain-load distribution reusing the `.dmn-bar` percentage-bar language); the week/month pace metrics (neutral-reference, delta only once the week closes); the domain filter; and the read-only calendar agenda + week event list, calling a `{WORKER_URL}/calendar/events` endpoint per the brief's §16 Worker companion spec. **The Worker-side calendar proxy itself is a separate deliverable** (lives in the Cloudflare Worker repo, not here) — until it's deployed, the agenda/week-event-list degrade to the "Calendar unavailable" placeholder while the rest of the screen renders normally, exactly as specced. Verified in-browser against a mocked Worker (real Notion/Worker access isn't reachable from a sandboxed build session — same "land, then verify live" caveat the brief itself calls out).
...
*(Note: this list predates several sessions' worth of shipped work logged elsewhere in this doc and in git history — e.g. the Edit Slice and the Today dashboard above are both already built despite still being listed here. Treat this numbered list as stale pending a full reconciliation pass, not as the current source of truth for what's left.)*
