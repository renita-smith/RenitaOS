...
Weekly Review is a **dashboard** — chrome yes (the shared nav rail / wordmark / Capture affordance), the `.shell` three-zone profile grid **no** (Addendum 2 Scope). Unlike Today, this screen has no glance-rail content (no agenda/domain-load column to fill) — so, the same shape as **Inbox** (which also carries no right rail), the 230px properties column is **dropped entirely** for this one route and its width goes to the main content column instead. The framed panel keeps the exact same max-width/cream-panel/deeper-page treatment as every other screen — only the internal rail/main split changes. **Capture renders inline in the header** (top-right), matching where it actually sits on Inbox, not Today's sidebar (which only makes sense there because Today's sidebar has real content in it). Everything else — the full three-act sequence (Review → Reflect → Plan), **including** Act 3's two-week calendar and six 1% goal boxes — runs top to bottom in one main column; the standalone's local two-up (calendar beside goals) didn't survive contact with a real two-week calendar grid and was retired in favor of a straight stack.
