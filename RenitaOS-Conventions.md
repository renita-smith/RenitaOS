...
Weekly Review is a **dashboard** — chrome yes (the shared nav rail / wordmark / Capture affordance), the `.shell` three-zone profile grid **no** (Addendum 2 Scope). "No `.shell` grid" means no Progress/Dates/Properties glance *content* in the right rail — it does **not** re-home the Capture button: the standard three-column frame (rail / main / right rail) stays exactly as Today's, and the right rail carries only the shared Capture affordance, so the button is pixel-identical across every framed screen rather than a per-screen reimplementation. Everything else — the full three-act sequence (Review → Reflect → Plan), **including** Act 3's two-week calendar and six 1% goal boxes — runs top to bottom in one main column; the standalone's local two-up (calendar beside goals) didn't survive contact with a real two-week calendar grid and was retired in favor of a straight stack. Since there's no glance-rail content competing for room, the framed panel drops its usual 1200px cap on this one route, reading as wide as Inbox's own unframed `.shell` instead of Today's narrower capped column.

The title + week-nav header sticks to the top of the main column on scroll (Capture already does, via the right rail's own sticky behavior) — same spirit as Inbox's own sticky bin header (Addendum 3).
