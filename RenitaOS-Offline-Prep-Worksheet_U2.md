...
| ✅ Loose tasks | Relation List | Loose tasks | The domain's **catch-all `Inbox/Admin` project** contents (§4 page IDs) — **NOT** a `Domain = this AND Projects empty` filter (that filter is void; a task's domain is a rollup through its project). Tap → Task profile |

> ⚠️ **Layout:** these profile tables list *which fields exist and where they drill*; the on-screen **arrangement** now follows Conventions **Addendum 2 — Framed Layout** (three-zone shell, properties rail, Progress → Dates → Properties), which supersedes the Meta/pill placement implied by the 'Role' column.
...
> Tags relate to Notes, People, Collections, Resources — **not Tasks.** ⚠️ **Projects:** the Tags↔Projects relation is phantom (Projects DB has no Tags relation — confirmed). Verify whether a one-directional `Tags → Projects` exists; if not, drop the `🚧 Projects` row above and the Projects list from this profile.
